...
# Azure / az-cli
# Ensures the az session is valid, then activates PIM roles for a
# subscription group defined in ~/.config/pim/groups.conf (default group: test).
#   pim        -> test group
#   pim prod   -> prod group
pim() {
  morning-pim.sh "${1:-test}" "${2:-Epiroc - LZ - Contributor}" "${3:-PT8H}"
}
